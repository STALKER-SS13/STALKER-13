...
/obj/structure/simple_extractor
	name = "seed grinder"
	desc = "A crude grinding machine which produces seeds when vegetation is put through it. Plants go in, seeds come out."
	icon = 'icons/obj/hydroponics/equipment.dmi'
	icon_state = "simplesextractor"
	density = TRUE
	anchored = TRUE

/obj/structure/simple_extractor/proc/seedify(obj/item/O, t_max, obj/structure/simple_extractor/extractor, mob/living/user)
	var/t_amount = 0
	var/list/seeds = list()
	if(t_max == -1)
...
/obj/structure/simple_extractor/attackby(obj/item/O, mob/user, params)
